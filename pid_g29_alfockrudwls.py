...
sys.path.append("/home/hglee/g29_ws/src/g29_force_feedback/msg")
from sensor_msgs.msg import Joy
from g29_force_feedback.msg import ForceFeedback

ctl_cmd=ForceFeedback()
Force = rospy.Publisher('ff_target', ForceFeedback, queue_size=1)
...
def joy_callback(joy_msg):
    ...
    Force.publish(g29msg)
    # now = rospy.Time.now()                                                                                                                     
    # rospy.loginfo("now: %f", now.to_sec())

def joycarla_callback(joy_msg):
    print('joycarla')
    if joy_msg.axes[2] >= 0:
        accel = (1.0+joy_msg.axes[2])/2
    else:
        accel = (joy_msg.axes[2]/2) +0.5

    if joy_msg.axes[3] >= 0:
        bake = (1.0+joy_msg.axes[3])/2
    else:
        bake = (joy_msg.axes[3]/2) +0.5

    vehicle_ctrl_info.brake = bake
    vehicle_ctrl_info.throttle = accel
    vehicle_ctrl_info.steer = joy_msg.axes[0] * -1.0
    vehicle_ctrl_info.reverse =  joy_msg.buttons[17] 
    #ctl_cmd.angle = 0
    #ctl_cmd.force = 0
    #ctl_cmd.pid_mode = True
    #Force.publish(ctl_cmd)
...
def find_waypoind(w1,w2) :
    ...
    grp.setup()
    spawn_points = world.get_map().get_spawn_points()
    #a = carla.Location(spawn_points[w1].location)#351 120
    a = w1.get_location()
    b = carla.Location(spawn_points[w2].location)#367 123
    w_route = grp.trace_route(a, b) 

    world.debug.draw_point(a,color=carla.Color(r=0, g=0, b=255),size=0.4 ,life_time=200.0)
    world.debug.draw_point(b,color=carla.Color(r=0, g=0, b=255),size=0.4 ,life_time=200.0)
...
def drive_through_plan(planned_route,speed,vehicles,PID):
    ...
    global lock,ii,z,time0,gTimestempS
    ...
    while not rospy.is_shutdown():
        ...
        #print('target',target)
        #print('x',vehicle_loc.x)
        #print('y',vehicle_loc.y)
        ...
        if (distance_v<3.5):
            ...
            ctl_cmd.angle = (control.steer)
            #ctl_cmd.force = (control.throttle)
            ctl_cmd.pid_mode = True
            print('distance_v<3.5')
        ...
        if cdata[2] !=6:
            ...
            carla.VehicleControl().steer =0.5
            vehicles = carla.VehicleControl().steer
            carla.VehicleControl().brake =0.1
            vehicles = carla.VehicleControl().brake 
            carla.VehicleControl().throttle=0.0
            vehicles = carla.VehicleControl().throttle
            gTimestempS = (datetime.now().strftime('%M:%S.%f'))
            print('gTimestempS',gTimestempS)
...
def Ack_Vehilce_ctrl(veldata, biodata) :
    if( biodata[0] == 1):
        if( ((veldata)-20) > 5 ):
            ...
            ack_vehicle_ctrl_info.acceleration = 0#abs((veldata/SPEED_OFFSET) - (20/SPEED_OFFSET)) #0.6208; #2km
    elif( biodata[0] == 2):
        ...
        if( biodata[1] == 1) :
            ...
            ack_vehicle_ctrl_info.speed =  0;
            ack_vehicle_ctrl_info.acceleration =abs((veldata/SPEED_OFFSET) - (10/SPEED_OFFSET))
        else:
            ack_vehicle_ctrl_info.speed = 20 / SPEED_OFFSET
            ack_vehicle_ctrl_info.acceleration =  0#abs((veldata/SPEED_OFFSET) - (10/SPEED_OFFSET)); #2km
    #elif( biodata[0] == 4):
        #vehicle_cmd.publish(vehicle_ctrl_info)
...
async def can_rc():
    ...
    print('엑셀 ',  gCanDataResult[3] / 100)
    print('emotion' ,gCanDataResult[0])
    print( 'user_emotion', gPotEmotion)
    date_s = (datetime.now().strftime('%M:%S.%f'))
    print('time',date_s)
...
def main():
    ...
    #rospy.Subscriber("joy", Joy, joycarla_callback, queue_size=1)
    #Force.publish(ctl_cmd)
    ...
    while not rospy.is_shutdown():
        ...
        if( flag == 0 ) :
            ...
            #print('debug_3')
            Ack_Vehilce_ctrl(current_speed, can_data)
            ack_vehicle_cmd.publish(ack_vehicle_ctrl_info)
            gTimestempE = (datetime.now().strftime('%M:%S.%f'))
            print('gTimestempE',gTimestempE)
            #time.sleep(2)
            #lock = 1
            #print('ack2')
