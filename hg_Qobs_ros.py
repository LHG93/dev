...
def sensor_callback(sensor_data, sensor_queue):
    ...
    #sensor_data.convert(carla.ColorConverter.Raw) #hum.....
    print(type(sensor_data.raw_data))
    i = np.array(sensor_data.raw_data)
    i2 = i.reshape((720,1280,4))
    #print ("2", i2.shape)
    #print ("")
    i3 = i2[:, :, :3]
    #cv2.imshow("s_frame",i3) #after remove
    #cv2.waitKey(1)
...
def cam(client,world):
    ...
    return camera, vehicle


    #actor_list.append(actor_list)
    #actor_list.append(camera)

def obs_callback(obs,web_queue):
    print("Obstacle detected:\n"+str(obs)+'\n')
    print(obs.transform,obs.actor,obs.other_actor,'%f'% obs.distance)
    web_queue.put(obs)
...
def RosSubCallbak_DataCarla(data):
    ...
    b=len(data.data)
    #print(data.step)
    #a=data.encoding.convert(carla.ColorConverter.Raw)
    #print(data.encoding)
    #print(data.height)
    #print(data.width)
    #self.json_str = json_message_converter.convert_ros_message_to_json(data)
    print('data.data',type(data.data))
    i5 = np.frombuffer((data.data), dtype=np.uint8)
    #i = np.array(i5)
    i2 = i5.reshape((720,1280,4))
    #i2 = i.reshape((720,1280,4))
    i3 = i2[:, :, :3]
    cv2.imshow("s_frame",i3) #after remove
    cv2.waitKey(1)
...
def hg_rostest():
    rospy.init_node("carla_hgtest", anonymous=False)
    rospy.Subscriber("/image_raw", 
    Image, RosSubCallbak_DataCarla )
...
def main():
    ...
    camera, vehicle = cam(client,world)

    obs_bp = world.get_blueprint_library().find('sensor.other.obstacle')
    #obs_bp_num = obs_bp[0].id
    #obs_bp = world.get_actors().find(obs_bp_num)
    obs_bp.set_attribute("distance", str(5.0))
    obs_bp.set_attribute("hit_radius", str(0.5))
    obs_bp.set_attribute("only_dynamics",str(True))
    obs_bp.set_attribute("debug_linetrace",str(True))
    obs_bp.set_attribute("sensor_tick",str(0))
    obs_location = carla.Location(0.0,0,0.0)
    obs_rotation = carla.Rotation(0,0,0)
    obs_transform = carla.Transform(obs_location,obs_rotation)
    obs = world.spawn_actor(obs_bp,obs_transform,attach_to=vehicle, attachment_type=carla.AttachmentType.Rigid)
    ...
    #obs.listen(lambda data: obs_callback(data, web_queue))
    #print('1')
    while not rospy.is_shutdown(): #True:
        ...
        try:
            print('hi!!!!')
            s_frame = sensor_queue.get(True, 1.0)
            #o_frame = web_queue.get(True, 1.0)
            #obs.listen(lambda data: obs_callback(data))
            #print(obs)
            #print(o_frame)
